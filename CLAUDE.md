...
Each page under `src/content/legal/*.md` is also published as a PDF at
`/legal/<slug>.pdf`. The PDFs are generated by an offline script
([scripts/generate-legal-pdfs.mjs](scripts/generate-legal-pdfs.mjs)) and
written to `public/legal/<slug>.pdf` so Cloudflare Pages serves them as
static assets alongside the HTML page.

```sh
npm run pdfs                          # rebuilds the site, then renders all 13 legal docs to PDF
npm run pdfs -- --skip-build          # reuse existing dist/, useful when iterating on the script
npm run pdfs -- --slug=reseller-europe   # regenerate one file
```

The script uses Puppeteer to print each rendered legal page (after hiding
the marketing chrome) to A4 PDF, then post-processes with `pdf-lib` to
overlay AcroForm text fields wherever the markdown contains a run of four
or more underscores. The two reseller agreements (`reseller-world.md`,
`reseller-europe.md`) are the only docs that currently have such runs, so
their PDFs ship with fillable signing/details fields; the other docs are
plain static PDFs.

Workflow when legal text changes:

1. Edit the relevant `src/content/legal/*.md` file.
2. Run `npm run pdfs` (regenerates all PDFs from the new build).
3. Commit both the markdown change and the regenerated PDFs in `public/legal/`.

The "Download PDF" button on each `/legal/<slug>` page is rendered
unconditionally by [LongForm.astro](src/layouts/LongForm.astro) and links
to `/legal/<slug>.pdf`. If the PDF is missing (script not yet run for a
new doc), the button 404s; nothing else breaks.

PDF coordinate conventions used by the script: A4 (595.28 × 841.89 pt) with
20mm margins on all sides, viewport sized to the printable area at 96 dpi
so that `getBoundingClientRect()` numbers map 1:1 to PDF points after the
print transform. Fillable rects that straddle a page boundary are snapped
to the next page (matches Chromium's "avoid breaking inline elements"
behaviour).
...
- **No analytics.** The site does not load Google Analytics or any other
  third-party tracker. If reintroduced, the [Cookies Policy](src/content/legal/cookies.md)
  and consent flow must be updated first; otherwise the site is in breach of
  UK PECR / EU ePrivacy.
