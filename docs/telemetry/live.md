...
Stalwart includes a Live Telemetry feature that provides administrators with real-time visibility into server activity. This feature exposes an HTTP endpoint that uses Server-Sent Events (SSE), also known as Event Streams, to deliver a continuous flow of structured logs, traces, and metrics directly to the client. Administrators can use this endpoint for live diagnostics, performance monitoring, and issue resolution.
...
It is recommended to access this feature through the [WebAdmin](/docs/management/webadmin/overview) interface. The WebAdmin tool includes a Dashboard that formats these live events. With WebAdmin, administrators can manage the live stream, apply filters, and visualize server activity.
