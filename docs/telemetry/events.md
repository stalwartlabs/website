...
|`housekeeper.schedule`|Housekeeper task scheduled|`INFO`|
...
|`queue.scheduled`|Message scheduled for delivery|`INFO`|
...
|`server.shutdown`|Shutting down Stalwart Mail Server v0.9.0|`INFO`|
|`server.startup`|Starting Stalwart Mail Server v0.9.0|`INFO`|
...
|`store.ingest`|Message ingested|`INFO`|
|`store.ingest-duplicate`|Skipping duplicate message|`INFO`|
|`store.ingest-error`|Message ingestion error|`ERROR`|
...
|`tracing.journal-error`|Journal collector error|`WARN`|
|`tracing.log-error`|Log collector error|`WARN`|
|`tracing.otel-error`|OpenTelemetry collector error|`WARN`|
|`tracing.update`|Tracing update|`DISABLE`|
|`tracing.webhook-error`|Webhook collector error|`WARN`|
...
- `cipher`: Encryption algorithm or cipher used
...
- `count`: Numerical count of items or occurrences
...
- `duration`: Time duration of an operation or event
...
- `minLearns`: Minimum number of learning iterations
...
- `origin`: Origin or source of an action or data
- `parameters`: Parameters or arguments for an operation
- `path`: File path or URL path
- `policy`: Policy name or identifier
- `policyType`: Type of policy applied
- `property`: Name of a property or attribute
- `protocol`: Communication protocol used
- `query`: Database query or search query
...
- `sourceAccountId`: Identifier of the source account
- `sourceMailboxId`: Identifier of the source mailbox
- `sourceUid`: Unique identifier from the source
- `spam`: Indicator or score for spam
- `spamLearns`: Number of spam messages learned
- `spanId`: Identifier for a span in distributed tracing
