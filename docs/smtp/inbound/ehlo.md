...
- `require`: Specifies whether the remote client must send an `EHLO` command before starting an SMTP transaction. It is recommended to set this value to `true` to allow Stalwart to verify the [SPF](/docs/smtp/authentication/spf) EHLO identity of the client.
