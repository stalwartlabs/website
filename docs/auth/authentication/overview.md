...
Stalwart supports authenticating users against several [backends](/docs/auth/backend/overview), which allows integration with existing user management systems. These include LDAP (Lightweight Directory Access Protocol), suitable for organizations that use an LDAP directory for storing user information, and SQL-based databases such as PostgreSQL, MySQL, and SQLite. Stalwart also provides an internal directory for user management directly within the mail server, which simplifies setup and administration for organizations of all sizes.
