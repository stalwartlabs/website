...
The information used for both authentication and authorization is stored in [directories](/docs/auth/backend/overview). A directory holds accounts, passwords, groups, mailing lists, and other metadata needed to manage and authenticate users and resources.

Directories contain [principals](/docs/auth/principals/overview), a term that refers to individuals, groups, resources, and other entities represented within the server. Principals form the basis on which the server models different roles and resources.
