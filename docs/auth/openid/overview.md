...
OpenID Connect is an addition to the OAuth 2.0 framework that provides authentication on top of the authorization flow. Stalwart provides flexible OpenID Connect support by acting as an OIDC provider, allowing clients to authenticate users and obtain identity information directly from Stalwart. Alternatively, it can be configured to authenticate users against third-party OIDC providers, integrating with external identity services. This lets Stalwart meet the authentication needs of a wide range of environments, whether by managing identities internally or using existing external identity providers.
