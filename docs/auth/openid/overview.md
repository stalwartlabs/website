...
OpenID Connect (OIDC) is an identity layer built on top of the [OAuth](/docs/auth/oauth/overview) 2.0 protocol. Where OAuth 2.0 is concerned with authorization (granting applications access to resources), OpenID Connect adds authentication: clients can verify the identity of the user who signed in. This supports single sign-on, in which a user authenticates once and uses the resulting identity across several services.
