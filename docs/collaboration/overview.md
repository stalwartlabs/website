...
For deployments that run Stalwart strictly as a mail server, instructions for [disabling WebDAV access](/docs/http/webdav/overview#disabling-webdav) are provided in the WebDAV documentation. WebDAV can be disabled globally or on a per-user, group, or tenant basis using the permission system.
