...
The spam classifier is configured on the [SpamClassifier](/docs/ref/object/spam-classifier) singleton (found in the WebUI under <!-- breadcrumb:SpamClassifier --><!-- /breadcrumb:SpamClassifier -->). The [`model`](/docs/ref/object/spam-classifier#model) field selects which classifier backend to run, or disables classification entirely. The available variants are:
