...
Stalwart can sit behind HAProxy for any of its listener protocols. The example below uses TCP-passthrough on every port (HAProxy hands TLS to Stalwart untouched), but HAProxy can equally terminate TLS for HTTP traffic and forward plain HTTP to Stalwart's port `8080`. Both patterns produce the same OAuth, OIDC, and JMAP discovery responses; see [How discovery URLs are composed](/docs/server/reverse-proxy/overview#how-discovery-urls-are-composed) for details and the [`STALWART_HTTPS_PORT`](/docs/configuration/environment-variables#public-urls) variable for non-standard public ports.
