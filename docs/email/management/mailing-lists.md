...
Mailing lists are defined by the [MailingList](/docs/ref/object/mailing-list) object (found in the WebUI under <!-- breadcrumb:MailingList --><!-- /breadcrumb:MailingList -->). Each instance represents one distribution address; the object stores the primary local part, the domain it attaches to, an optional description, the set of recipients, and any alternate addresses that should resolve to the same list.
