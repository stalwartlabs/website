...
Stalwart's contact management system is integrated with the [built-in spam filter](/docs/spamfilter/) to improve filtering accuracy and reduce false positives. When enabled, the spam filter references an account's address book to identify trusted senders so that their messages are not marked as spam.
...
For details on configuring these features, see the [spam filter documentation](/docs/spamfilter/).
