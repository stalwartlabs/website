...
- **WebUI and other hosted [Applications](/docs/management/applications/)**: the server downloads the WebUI bundle from `https://github.com/stalwartlabs/webui/releases/latest/` (and any additional Application bundles from their respective [`resourceUrl`](/docs/ref/object/application#resourceurl)). When the first download fails, the `/admin` and `/account` paths return `404 Not Found` because no bundle has been unpacked locally yet. See the WebUI's [Outbound network requirement](/docs/management/webui/#outbound-network-requirement) section for the WebUI-specific symptom and fix, and the general [outbound network requirement](/docs/management/applications/#outbound-network-requirement) note for any additional hosted Applications.
