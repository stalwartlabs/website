...
Authentication uses the ESMTP `AUTH` command with a SASL mechanism, one of `PLAIN`, `LOGIN`, `OAUTHBEARER` or `XOAUTH2`, the initial response supplied inline or after a `334` continuation. `AUTH LOGIN` is advertised on the `EHLO` response whenever `LOGIN` appears in the `sasl` list and cleartext authentication is permitted on the connection, which keeps clients that only implement `LOGIN`, such as Outlook 2019 for macOS, working through the proxy. The proxy sends the base64 `Username:` and `Password:` challenges itself, accepts the base64 username as an initial response when the client sends one, honours `*` as an abort, and reconstructs the exchange as a `PLAIN` challenge, so the backend leg only ever sees `PLAIN`. Cleartext `PLAIN` and `LOGIN` are gated on transport security and refused with an encryption-required response until the connection is secured, unless `allow_plain_auth_without_tls` is set. The routing identifier is extracted from the credentials as described under [Routing](/docs/migration/proxy/routing/).
