...
Local accounts are created from the external directory on demand rather than being copied in advance, and they are never removed automatically. Where the identity provider should own the whole account lifecycle, including suspension and deletion, [SCIM provisioning](/docs/auth/scim/) can be enabled per domain; [Provisioning models](/docs/auth/scim/provisioning) sets out how the two approaches differ and how authority is assigned between them.
