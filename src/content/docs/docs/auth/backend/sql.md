...
Accounts are not copied from the database in advance. Stalwart materialises a local account the first time it needs one, either when the user authenticates or when a message arrives for their address and [`queryRecipient`](/docs/ref/object/directory#queryrecipient) confirms it as a valid recipient. Because that query can be run at any time, an account that has never signed in can still receive mail.

Removal is the case this does not cover. Just-in-time provisioning never deletes anything: a row removed from the `accounts` table stops matching queries, but the mailbox Stalwart created from it remains, keeps accepting mail, and keeps counting against licensed mailbox limits.
