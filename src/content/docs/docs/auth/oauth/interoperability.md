...
OAuth is widely used in web-based applications to provide secure, token-based authentication and authorization. In Stalwart, OAuth can be used to access [JMAP](/docs/http/jmap/) as well as the [WebUI and self-service portal](/docs/management/webui/), which are web-based interfaces that natively support OAuth flows. This usage matches the typical OAuth pattern: users authenticate through a browser, and the application exchanges authorization artefacts for access tokens with the OAuth server.
