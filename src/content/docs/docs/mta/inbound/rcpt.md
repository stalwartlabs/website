...
Handling of local accounts relies on a configured [directory](/docs/auth/backend/), which performs the following roles:
...
The [`script`](/docs/ref/object/mta-stage-rcpt#script) field selects the [Sieve script](/docs/sieve/) to run at this stage. A common use is greylisting, implemented by storing a `remote_ip.sender.recipient` triplet in a store and rejecting the first attempt with a `422` temporary failure; the client then retries, and legitimate senders succeed while most spam sources do not.
