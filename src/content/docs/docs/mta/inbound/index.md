...
Each stage is configured by its own singleton so that specific behaviour can be applied at each point in an SMTP session. Most stages support running [Sieve scripts](/docs/sieve/), allowing custom filters that reject, alter, or reroute incoming messages based on a wide range of criteria.
