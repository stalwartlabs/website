...
- `sql`: SQL database
- `ldap`: LDAP server
- `oidc`: [OpenID](/docs/auth/openid/) Connect server
...
The default directory to authenticate [JMAP](/docs/http/jmap/), [IMAP](/docs/0.15/email/imap), [ManageSieve](/docs/sieve/managesieve) and HTTP API requests against can be configured using the `storage.directory` configuration attribute.
