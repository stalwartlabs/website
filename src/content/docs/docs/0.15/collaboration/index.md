...
For administrators who want to run Stalwart strictly as a mail server, we provide clear instructions for [disabling WebDAV access](/docs/http/webdav/#disabling-webdav). This can be done globally or on a per-user, group, or tenant basis, allowing complete control over which services are exposed to clients.
