...
- `enable`: Determines whether this hook is turned on or off. This setting can be dynamically set using [expressions](/docs/configuration/expressions/) which allows a certain Milter to be enabled or disabled based on the specific circumstances of an SMTP transaction.
...
Please refer to the [MTA Hooks API](/docs/0.15/api/mta-hooks/) documentation for detailed information on the MTA Hooks API, including request and response formats, as well as examples.
