...
- [OAuth authorization codes](/docs/auth/oauth/) to validate the authorization process.
- [ACME tokens](/docs/server/tls/acme/) for SSL/TLS certificate management.
- Certain e-mail tracking data, such as [greylisting](/docs/spamfilter/greylist) tokens and [Sieve](/docs/sieve/) auto-responder IDs.
...
Although Stalwart requires a default in-memory store, it is possible to define multiple in-memory stores to be used from expressions](/docs/configuration/expressions/) and [Sieve scripts](/docs/sieve/). 
