...
Stalwart includes a built-in HTTP server, which primarily serves two functions: handling [JMAP](/docs/http/jmap/), [WebDAV](/docs/http/webdav/) requests and processing requests to the [REST management API](/docs/0.15/api/management/). The following settings can be configured to customize the behavior of the HTTP server.

## URL

The `http.url` setting is an [expression](/docs/configuration/expressions/) that specifies the URL of the HTTP server. It is essential for guiding clients to the correct address when they make `.well-known` requests for the JMAP (JSON Meta Application Protocol) and OAuth resources. It is returned to clients as part of the discovery process, enabling them to configure themselves with the appropriate service URLs for JMAP access or initiating OAuth authentication flows.
