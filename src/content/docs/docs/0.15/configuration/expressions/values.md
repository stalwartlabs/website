...
These variables refer to the data matched by a regex or other [expressions](/docs/configuration/expressions/). They are denoted as `$n`, where `n` is a number that represents the position of the matched group in a regular expression. For instance:
