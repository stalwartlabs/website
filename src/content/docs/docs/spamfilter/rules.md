...
Stalwart supports custom spam filter rules that analyse incoming messages and assign tags dynamically. Rules use [expressions](/docs/configuration/expressions/) to evaluate specific parts of a message (headers, body, sender information, attachments, and so on) and to decide which tags to apply.
