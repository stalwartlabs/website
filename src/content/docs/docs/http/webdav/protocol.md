...
- [`requestMaxSize`](/docs/ref/object/web-dav#requestmaxsize): the maximum XML size of a WebDAV request that the server will accept, in bytes. Default `26214400` (25 MB). This limit matters most for file uploads, calendar imports, and contact synchronisation, which tend to carry large payloads. Administrators can raise it to support larger transfers or lower it to reduce resource consumption.
