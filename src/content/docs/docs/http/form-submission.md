...
- [`maxSize`](/docs/ref/object/http-form#maxsize): maximum size of a single submission, in bytes. Default `102400` (100 KB).
...
- [`rateLimit`](/docs/ref/object/http-form#ratelimit): per-IP submission rate, as a `count` over a `period` given in milliseconds. Default five submissions per hour.
- [`deliverTo`](/docs/ref/object/http-form#deliverto): the set of local email addresses that receive the generated message.
